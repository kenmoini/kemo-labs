...
- `192.168.42.13` (shared by both services on different ports)
...
- Static IP `192.168.42.13` on the homelab macvlan/ipvlan network.
...
3. **iPerf3 direct access:** iPerf3 clients connect directly via `iperf3 -c 192.168.42.13`. There is no web interface for iPerf3 -- it is a CLI tool.
...
5. **Network accuracy:** For the most accurate results, avoid running speed tests through the proxy chain. Access OpenSpeedTest directly at `http://192.168.42.13:3000` for baseline measurements, and through Traefik to measure proxied performance.
